...
function getDarkLetterColor() {
 const darkColors = ['#111111', '#222222', '#333333', '#000000'];
...
function makeIconSVG({ name, bg, hinge, letterColor }) {
 ...
 let fontSize = 7;
 if (bookName.length > 12) fontSize = Math.max(4, Math.floor(7 - (bookName.length - 12) * 0.3));
...
<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${letterX}" y="${letterY}" font-family="'Vast Shadow', serif" font-weight="400" font-style="normal" font-size="45" fill="${letterColor}" text-anchor="end" dominant-baseline="alphabetic" text-rendering="geometricPrecision">${firstLetter}</text>
</svg>`;
}
